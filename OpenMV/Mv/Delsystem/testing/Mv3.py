...
def laneAppropriateImgBoth(img_copy_mid, threshold_, pixelthreshold_, xstride_, ystride_, margin_):
    ...
    return img_copy_mid, img_copy_side, pedestrianCrossingFiltered
...
while True:
    ...
    while True:
        ...
        laneAppropiateMiddle, laneAppropiateSide, allCrossings = laneAppropriateImgBoth(laneAppropriateImg(img, [obstacle]), [(255,255)], 325, 4, 4, 15)
        ...
        if middleCrossing or leftCrossing and rightCrossing:
            leftLaneLine = getLaneLine(laneAppropiateSide, [(255, 255)], 20, True, 4, 4, LEFT_LANE_ROI_WALK) # WRONG ROI 123
            rightLaneLine = getLaneLine(laneAppropiateSide, [(255, 255)], 20, True, 4, 4, RIGHT_LANE_ROI_WALK)
            bothV, bothX, new = getSteerValues([leftLaneLine, rightLaneLine], bothV, bothX)
            drawLine(img, [leftLaneLine, rightLaneLine], (0, 0, 0), 2)
        else:
            middleLaneLine = getLaneLine(laneAppropiateMiddle, [(255, 255)], 20, True, 4, 4, MIDDLE_LANE_ROI) # WRONG ROI 123
