...
def laneAppropriateImgBoth(img_, threshold_, pixelthreshold_, xstride_, ystride_, margin_):
    img_copy = img_.copy()
    ...
    outlineObjects(img_copy, pedestrianCrossingFiltered, 0, 1, True)
    img_copy.erode(1)
    img_copy.dilate(1)

    sideLinesFiltered = []
    sideLines = getColoredObjects(img_, threshold_, 2, xstride_, ystride_, 2, ALL_ROI)
    for roi in sideLines:
        if roi[4] <= 225:
            sideLinesFiltered.append(roi)

    outlineObjects(img_copy, sideLinesFiltered, 0, 1, True)
    img_copy.erode(1)
    img_copy.dilate(1)

    return img_copy, pedestrianCrossingFiltered
...
while True:
    ...
    while True:
        ...
        laneAppropiateAll = laneAppropriateImg(img, [obstacle])
        laneAppropiateMiddle, allCrossings = laneAppropriateImgBoth(laneAppropiateAll, [(255,255)], 325, 4, 2, 10)
        leftCrossing, middleCrossing, rightCrossing = getPedestrianCrossings(allCrossings)
        # leftCrossing = getPedestrianCrossing(laneAppropiate, [(255, 255)], 4, 2, LEFT_LANE_ROI_WALK, 50)
        # rightCrossing = getPedestrianCrossing(laneAppropiate, [(255, 255)], 4, 2, RIGHT_LANE_ROI_WALK, 50)
        # middleCrossing = getPedestrianCrossing(laneAppropiate, [(255, 255)], 4, 2, MIDDLE_LANE_ROI_WALK, 50)

        if middleCrossing or leftCrossing and rightCrossing:
            leftLaneLine = getLaneLine(laneAppropiateAll, [(255, 255)], 20, True, 4, 2, LEFT_LANE_ROI_WALK) # WRONG ROI 123
            rightLaneLine = getLaneLine(laneAppropiateAll, [(255, 255)], 20, True, 4, 2, RIGHT_LANE_ROI_WALK)
            bothV, bothX, new = getSteerValues([leftLaneLine, rightLaneLine], bothV, bothX)
            drawLine(img, [leftLaneLine, rightLaneLine], (0, 0, 0), 2)
        else:
            middleLaneLine = getLaneLine(laneAppropiateMiddle, [(255, 255)], 20, True, 4, 2, MIDDLE_LANE_ROI) # WRONG ROI 123
